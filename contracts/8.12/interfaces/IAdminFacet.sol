...
interface IAdminFacet {
  ...
  function setRiskManagersOk(
    address[] calldata _riskManagers,
    bool _isOk
  ) external;
  ...
  function setRepurchaseRewardModel(
    IFeeModel _newRepurchaseRewardModel
  ) external;
  ...
  function setProtocolConfigs(
    ProtocolConfigInput[] calldata _protocolConfigInput
  ) external;

  function setLiquidationParams(
    uint16 _newMaxLiquidateBps,
    uint16 _newLiquidationThreshold
  ) external;
